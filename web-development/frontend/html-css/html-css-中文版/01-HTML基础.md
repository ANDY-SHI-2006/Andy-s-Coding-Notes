...
> **最佳实践：** 每个页面只使用一个 `<h1>`。标题应遵循逻辑层级（`h1` → `h2` → `h3`），不要跳级。`<h1>` 其实可以写多个——合法且浏览器照常渲染，但多个 `<h1>` 会干扰屏幕阅读器跳转和搜索引擎判断页面主题，所以约定一页一个。
...
> **注意：** 不要在 `<p>` 内嵌套块级元素（如 `<div>`、`<h1>`–`<h6>` 或另一个 `<p>`）。浏览器会自动闭合段落，导致排版异常。应使用 `<div>` 或 `<span>` 作为嵌套文本容器。这种自动纠错只针对 `<p>` 等少数特定标签；其他非法嵌套（如 `<span>` 套 `<p>`）浏览器不会纠正、会原样渲染，但同样属于非法写法，要避免。
