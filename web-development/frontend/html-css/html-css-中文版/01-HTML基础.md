...
#### 1.1.2.3.1 块级（`<div>`、`<p>`、`<h1>`~`<h6>`、`<ul>`）
- 独占一行——就算有空余位置，也不让其他标签使用。
- 宽高可以自由设置。
- 宽度未设置时继承上一级的宽度；高度未设置时由内容决定。
- 可以嵌套其他标签；但 `<p>` 不能嵌套块级标签（嵌套了浏览器会自动拆开）。
- 内外边距都生效（详见 CSS 章节）。

#### 1.1.2.3.2 行级（`<span>`、`<a>`、`<strong>`、`<em>`）
- 不独自占一行，可以和其他行级元素共享一行。
- 宽高不能自由设置，由内容决定。
- 只能嵌套文本或行级元素。
- 内外边距只在水平方向生效，垂直方向不生效（详见 CSS 章节）。

#### 1.1.2.3.3 行内块（`<img>`、`<input>`、`<textarea>`）
- 不独自占一行，可以和其他行级/行内块标签共享一行。
- 可以设置宽高。
- 未设置宽高时以内容为准。
- 内外边距都生效，和块级一样（详见 CSS 章节）。
