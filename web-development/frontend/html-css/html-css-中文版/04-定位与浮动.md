...
综合示例——纯色背景、双向平铺、no-repeat 居中以及 contain 与 cover 的尺寸差异（背景图用内联 SVG 数据 URI 实现，无需外部文件）：

```html
<style>
    /* 用内联 SVG 数据 URI 当背景图，无需外部文件 */
    .demo > div {
    ...
    .demo .solid { background: #fde68a; }               /* 纯色背景（简写会重置背景图） */
    .tile  { background-repeat: repeat; }               /* 默认值：水平垂直双向平铺 */
    ...
    .demo span { background: rgba(255, 255, 255, 0.85); }  /* 衬底保证文字可读 */
</style>
<div class="demo">
...
下面的综合示例对比左右浮动的文字环绕效果，以及父元素高度塌陷和两种修复方法的差异：
...
<style>
    ...
    .overflow-fix { overflow: hidden; }   /* 触发 BFC 包裹浮动子元素 */
    .clearfix::after { content: ""; display: block; clear: both; }  /* 伪元素清除浮动（推荐） */
</style>
<div class="row">
    <div>
...
综合示例如下：左侧面板中 Box B 相对其原始位置（虚线框）偏移，原空间仍被保留；右侧面板中 NEW 徽章和价格标签以 relative 父容器为参照进行绝对定位：
...
三种常用居中方法的效果对比如下（水平居中、绝对定位居中、Flexbox 居中）：
...
<style>
    ...
    .h-auto { width: 120px; margin: 0 auto; }   /* 水平居中：剩余空间由 auto 平分 */
    .pos-parent { position: relative; }
    .pos-item {
        position: absolute; top: 50%; left: 50%;
        transform: translate(-50%, -50%);   /* 按自身尺寸的一半回移 */
    }
