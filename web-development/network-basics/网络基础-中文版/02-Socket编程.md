...
UDP 常被称为"不可靠"协议，但"不可靠"不等于"不好用"。本节简要介绍 UDP 的几个进阶话题，帮助理解什么时候以及如何使用它。
...
| 现象 | 含义 | 容忍设计的例子 |
|------|------|----------------|
| **丢包** | 数据报可能在中途丢失，发送方收不到任何通知 | 视频直播丢一帧直接跳过，不重传 |
| **乱序** | 后发的数据报可能先到达 | 接收方给包编号，按序号重组 |
| **重复** | 同一数据报可能到达多次 | 接收方按序号去重 |

为什么 DNS、实时音视频、在线游戏仍然选择 UDP？因为 **低延迟** 比"偶尔丢一点"更重要：TCP 为了保证可靠会重传，重传带来的等待在实时场景里比丢失本身更糟糕。此外 UDP 的开销更小——头部只有 8 字节，而 TCP 至少 20 字节。

### 2.7.2 MTU 与分片

以太网链路的 **MTU（最大传输单元）** 通常是 1500 字节。UDP 数据报一旦超过这个限制，IP 层会把它 **分片** 成多个小包分别传输；接收方必须收齐所有分片才能还原，**任何一个分片丢失，整个数据报就作废**。

因此实践中建议 UDP 数据报不超过 **1472 字节**：

```
1500（MTU）- 20（IP 头）- 8（UDP 头）= 1472 字节
```

要传更大的数据，应在应用层自己拆分，而不是依赖 IP 分片。

### 2.7.3 广播与组播

UDP 支持 TCP 做不到的 **一对多** 通信：

- **广播（broadcast）**：向局域网内所有主机发送（如 `255.255.255.255`），socket 需先设置 `SO_BROADCAST` 选项。典型应用：局域网内的服务自动发现。
- **组播（multicast）**：向加入了特定组播组（`224.0.0.0` ～ `239.255.255.255`）的主机发送，只有感兴趣的接收方才收到。典型应用：IPTV、行情推送。

两者都建立在 UDP 的无连接特性之上；TCP 是点对点连接，无法广播。

### 2.7.4 在 UDP 之上重建可靠性：QUIC

UDP 的"不可靠"并非不可救药——应用层可以自己实现确认、重传、排序，需要多少可靠性就实现多少。这正是 **QUIC** 协议（HTTP/3 的底层，见 1.4.3）的思路：它运行在 UDP 之上，自己处理可靠传输、加密和连接管理，既保留了 UDP 的灵活与低延迟，又避免了 TCP"一丢包全队等待"的队头阻塞问题。

> **小结**：UDP 用"不保证"换来了低延迟、低开销和一对多能力。理解它的限制（分片、丢包、乱序），才能在对的场景用对它。
