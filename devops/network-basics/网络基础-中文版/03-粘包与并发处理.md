...
完整可运行示例：[非阻塞服务器](../examples/zh/nonblocking_server.py) · [客户端](../examples/zh/echo_client.py)
...
**什么时候算"可读"？** 一个 socket 满足以下任一情况就会被标记为可读就绪：

| 情况 | 出现在哪种 socket 上 | 对应的操作 |
|------|---------------------|-----------|
| 有新的连接请求到达（收到 SYN） | 监听中的服务器 socket | 调用 `accept()` 取走新连接，不会阻塞 |
| 接收缓冲区里有数据等着读 | 已连接的客户端 socket | 调用 `recv()` 读走数据 |
| 对端关闭了连接（收到 FIN） | 已连接的客户端 socket | `recv()` 立即返回空字节串（EOF） |

注意后两行的视角："建立连接"和"发消息"虽然都由客户端发起，但服务端程序感知到的就绪对象是**两个不同的 socket** ——前者是监听 socket（`server`），后者是 `accept()` 返回的、代表那个具体客户端的连接 socket（`conn`）。3.3.2 代码里 `if sock is server` 的分支判断，区分的就是这两种情况。
...
完整可运行示例：[select 服务器](../examples/zh/select_server.py) · [客户端](../examples/zh/echo_client.py)
