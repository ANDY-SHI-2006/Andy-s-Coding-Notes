...
### 3.1.5 使用长度前缀协议的服务器与客户端

首先，在 `util.py` 中定义一个辅助函数，用于精确读取指定数量的字节（`recv(n)` 一次可能读不满 `n` 个字节，需要循环）：
...
在实际项目中，可以在 3.1.5 的 `recv_exactly` 基础上，把发送和接收也封装成可复用的函数，并加上最大消息长度保护，得到完整的 `util.py`：
