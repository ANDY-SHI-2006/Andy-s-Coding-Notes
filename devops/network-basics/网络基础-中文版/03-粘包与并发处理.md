...
注意后两行的视角："建立连接"和"发消息"虽然都由客户端发起，但服务端程序感知到的就绪对象是**两个不同的 socket** ——前者是监听 socket（`server`），后者是 `accept()` 返回的、代表那个具体客户端的连接 socket（`conn`）。3.3.3 代码里 `if sock is server` 的分支判断，区分的就是这两种情况。
