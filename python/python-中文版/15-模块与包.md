...
**包**是一个捆绑相关模块的目录。它通常包含一个 `__init__.py` 文件（往往为空），以便 Python 将其识别为包而不是普通目录。

### 15.2.1 什么是包？

可以把包看作一个命名空间：`graphics.shapes` 将与图形相关的代码组织在 `graphics` 包之下，使名称井然有序并避免冲突。
...
如果没有 `__all__`，`from package import *` 会导入所有不以下划线开头的名称。在 `__init__.py` 中使用 `__all__` 来定义公开 API。
...
`pip` 是 Python 的包安装工具，PyPI（Python Package Index，Python 包索引）是公共包仓库。
