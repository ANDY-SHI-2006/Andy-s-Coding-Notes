...
set1 = {"Alice", "Bob"}
set1.add("Charlie")
print(set1)  # {'Alice', 'Bob', 'Charlie'} (顺序可能不同)
```
...
dict1["Unicom"] = 10010      # 添加新键
dict1["Mobile"] = 100861     # 更新已有键
print(dict1)
```
...
# 带默认值的安全 pop
value = dict1.pop("Unknown", 0) # value = 0，不报错
```
...
for key in dict1:  # 等同于 dict1.keys()
    print(key)     # A, B, C
...
counts["apple"] += 1  # 不会报 KeyError；使用默认值 0
...
```python
d = {"a": 1, "b": 2, "c": 3}
print(list(d.keys()))  # ['a', 'b', 'c'] (保持原有顺序)
```
...
# O(1) 成员测试
s = set(range(1000000))
print(999999 in s)   # True (快)

# O(n) 成员测试
lst = list(range(1000000))
print(999999 in lst) # True (较慢)
```
