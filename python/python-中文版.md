...
##### 3.0.0.2.1. 使用场景： 最适合简单条件赋值。复杂逻辑请使用标准 if-else 以保证可读性。
...
##### 3.0.0.5.1. 注意： 与三元运算符不同。单行 `if` 仅处理"真"情况，无 else 分支。
...
##### 3.0.0.6.1. 使用场景： 适用于需要知道是否未找到项目的搜索操作。
...
#### 4.0.0.4. 仅列表可用 - 一种灵活且强大的简洁创建列表方式。

#### 4.0.0.5. 语法：`[表达式 for 项目 in 可迭代对象]`
...
#### 4.0.0.7. `sorted()`：内置函数，对任何序列类型排序，支持升序和降序，**返回新序列**（原序列不变）。

#### 4.0.0.8. `.sort()`：**原地**修改列表（不推荐；请改用 `sorted()`）。
...
#### 4.0.0.9. 可变类型： 列表、字典、集合

- 创建后可修改
- 内部值改变，但**内存地址保持不变**

#### 4.0.0.10. 不可变类型： 整数、字符串、元组、布尔值、浮点数
...
#### 5.0.0.5. 结构： 键可为任何不可变类型；值可为任何类型（包括字典）。
...
##### 7.0.0.0.1. 注意： Lambda 不提升性能；复杂逻辑请使用 `def`。

## 7.2 高阶函数

### 7.2.1 简介

#### 7.0.0.1. 高阶函数： 以函数为参数或返回函数的函数。
...
#### 8.0.0.1. 闭包： 引用其封闭作用域变量并从外部函数返回的嵌套函数。
...
#### 8.0.0.2. 装饰器： 包装另一个函数以在不修改它的情况下扩展其行为的函数。
