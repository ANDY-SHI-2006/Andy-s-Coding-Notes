[‚Ü?Previous: Queue ADT](19-queue.md) | [Next: Sorting Algorithms ‚Üí](21-sorting-algorithms.md)
...
Big-O notation describes the upper bound of growth rate, focusing on the dominant term as n ‚Ü?‚à?
...
| **O(2‚Å?** | Exponential | Brute force combinations | Subset generation |
...
// Fibonacci (naive): O(2‚Å? time - very slow!
int fibonacci(int n) {
    if (n <= 1) return n;
...
1. **Nested loops** ‚Ü?Multiply complexities
   ```cpp
   for() { for() { } }  // O(n) * O(n) = O(n¬≤)
   ```

2. **Sequential statements** ‚Ü?Add, keep maximum
   ```cpp
   O(n) + O(n¬≤) = O(n¬≤)
   ```

3. **Loop with fixed iterations** ‚Ü?O(1)
   ```cpp
   for(int i = 0; i < 100; i++)  // O(1), not O(n)
   ```

4. **Divide by constant** ‚Ü?Logarithmic
   ```cpp
   while(n > 1) { n /= 2; }  // O(log n)
   ```

5. **Multiple variables** ‚Ü?State both
   ```cpp
   for(i=0; i<n; i++)
       for(j=0; j<m; j++)  // O(n * m)
   ```
...
2. **Drop constants and lower-order terms**: `2n¬≤ + 3n + 1` ‚Ü?`O(n¬≤)`
...
O(log n)    <‚îÄ‚îÄ Good
O(n)        <‚îÄ‚îÄ Fair
O(n log n)  <‚îÄ‚îÄ Acceptable
O(n¬≤)       <‚îÄ‚îÄ Poor (be careful with large n)
O(n¬≥)       <‚îÄ‚îÄ Very Poor
O(2‚Å?       <‚îÄ‚îÄ Terrible (avoid for n > 30)
O(n!)       <‚îÄ‚îÄ Impossible (avoid for n > 15)
```

### When Does It Matter?

For n = 1,000:
- O(log n): ~10 operations ‚ú?- O(n): 1,000 operations ‚ú?- O(n¬≤): 1,000,000 operations ‚ö†Ô∏è
- O(2‚Å?: More operations than atoms in the universe ‚ù?
[‚Ü?Previous: Queue ADT](19-queue.md) | [Next: Sorting Algorithms ‚Üí](21-sorting-algorithms.md)
